package com.example.apptravelbooking.helper;
...
public class TravelBookingDbHelper {
    ...
    public static void saveBooking(@NonNull Callback<BookingModel> saveSuccessCallback, BookingModel bookingModel) {
        db.collection("bookings").add(bookingModel.toMap()).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
            @Override
            public void onSuccess(DocumentReference documentReference) {
                documentReference.get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                    @Override
    ...
    public static void getTourById(@NonNull Callback<TourModel> tourModelCallback, int tourId) {
        db.collection("tours").whereEqualTo("TourId", tourId).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                tourModelCallback.callback(queryDocumentSnapshots.getDocuments().get(0).toObject(TourModel.class));
            }
        });
    }
    ...
    public static void seedTours() {
        db.collection("tours").count().get(AggregateSource.SERVER).addOnSuccessListener(new OnSuccessListener<AggregateQuerySnapshot>() {
            ...
            public void onSuccess(AggregateQuerySnapshot aggregateQuerySnapshot) {
                ...
                // If dont have any tours we should seed data for them
                if (numberOfTours == 0) {
                    ...
                    listOfTours.add(new TourModel(1, 1, "Tour 4 đảo Nam Phú Quốc - 1 ngày", "143 Trần Hưng Đạo, KP 7, TT Dương Đông, H.Phú Quốc, tỉnh Kiên Giang", 9.1F, 645000, 800000, "Tiết kiệm 20%",
                            ...
                            Arrays.asList("08:30-17:00 Đón khách", "Tham quan đảo Gầm Ghì, lặn ống thở và tắm biển", "Tham quan hòn Bườm hoặc hòn Móng Tay, lặn ống thở và tắm biển", "Tham quan hòn Mây Rút Trong và dùng bữa trưa", "Trải nghiệm dịch vụ đi dưới biển Seawalker (chi phí tự túc)", "Trả khách", "Kết thúc tour."),
                    ...
                    listOfTours.add(new TourModel(2, 1, "Tour khám phá 3 đảo tại Phú Quốc - 1 ngày", "Gam Ghi island, Hòn Thơm, Phu Quoc, Kien Giang", 8.7F, 550000, 650000, "Tiết kiệm 15%",
                            ...
                            Arrays.asList("08:30-17:30 Đón khách", "Tham quan đảo Gầm Ghì, lặn ống thở và tắm biển", "\n" +
